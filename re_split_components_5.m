% ------------------------------------------------------------------------
% Get ICA cleaned data, split back to separate awakenings, then manaully
% clean the data based on time course
...
%     save([subdir '/chan_spects.mat'],'chan_spects');
 ...
 while (true)
    ...
    for freq_l = 1:length(freq_bans)
        ...
        figure; topoplot([],EEG.chanlocs,'style','blank','electrodes','ptsnumbers','chaninfo',EEG.chaninfo);
        
        inputdlg_opts = struct('WindowStyle', 'normal'); % Stops focus stealing
        answer = inputdlg('Bad channels remaining: ', '', 1, {''}, inputdlg_opts);
        close();

        if isempty(answer) || isempty(answer{1})
            % No bad channels left.
            break;
        else
            % Bad channels were found, so update our list.
            add_bad_chans = str2num(answer{1});
            EEG = epi_log(@pop_select, EEG, 'nochannel', add_bad_chans);
            EEG_avgref = epi_log(@pop_select, EEG_avgref, 'nochannel', add_bad_chans);
        end
    end
    ...
    saveas(gcf, [sesdir '/' sprintf('awakening-%d-cleaned_nrem.tif', nrem_index(awak))], 'tif');
