function channels = mff_import_signal_binary_uncalibrated(signal_binary, channels_index, blocks_list)
...
%     calibrated_zeros       = signal_binary.calibrated_zeros(blocks_list, channels_index);
    ...
    for channel_num = 1:num_channels
        ...
        channels(channel_num).samples       = single(zeros(1, channel_num_samples(channel_num)));
    end
...
%             channels(channel_num).samples(sample_from:sample_to) = single(calibrated_gains(block_num, channel_num) * (samples(:) - calibrated_zeros(block_num, channel_num)));
            channels(channel_num).samples(sample_from:sample_to) = single(samples);
        end
    end
