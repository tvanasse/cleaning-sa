function [num_blocks, blocks, num_channels, channels] = mff_import_signal_binary_meta_data(file)
    ...
    id            = fopen(file, 'r', 'l');
    block_version = fread(id, 1, 'uint32'); % header start
    
    while ~feof(id)
        num_blocks = num_blocks + 1;
        
        if block_version > 0
            block_header_size     = fread(id, 1, 'uint32'); % 1
            block_data_size       = fread(id, 1, 'uint32'); % 2
            block_num_channels    = fread(id, 1, 'uint32'); % 3
            block_channel_offsets = fread(id, [1, block_num_channels], 'uint32'); % 4
            
            if num_blocks == 1
                num_channels              = block_num_channels;  % makes sure that each block has same number of channels
            ...
            for channel_num = 1:num_channels  % for each signal
                sample_size = fread(id, 1, 'uint8'); %4 + ns (signal depth)
                
                if sample_size ~= 32
                    error('unsupported %d-bit sample size for channel %d in block %d', sample_size, channel_num, num_blocks);
                end
                
                sampling_rate = fread(id, 1, 'ubit24'); % 4 + ns (signal frequency)
